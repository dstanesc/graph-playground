...
describe('Graph', function () {
    describe(`Custom Node ${offsetIncrements.node} bytes binary encoding/decoding`, function () {
        it('Node serde', function () {
            const nodes = [
                new Node(new Offset(0), "The first node", new Offset(1234), new Offset(3456)),
                new Node(new Offset(offsetIncrements.node), "The second node", new Offset(4321), new Offset(6543)),
                new Node(new Offset(offsetIncrements.node * 2), "The third node", new Offset(55555), new Offset(77777))
            ]

            const nodesByteArray = new NodesEncoder(nodes).write().content()

            assert.equal(offsetIncrements.node * 3, nodesByteArray.length)
            ...
            assert.equal(offsetIncrements.node, node2.offset.offset)
            assert.equal("The second node", node2.label)
            assert.equal(4321, node2.nextRlshp)
            assert.equal(6543, node2.nextProp)

            assert.equal(offsetIncrements.node * 2, node3.offset.offset)
    ...
    describe(`Custom Rlshp ${offsetIncrements.rlshp} bytes binary encoding/decoding`, function () {
        it('Rlshp serde', function () {
            const rlshps = [
                ...
                new Rlshp(new Offset(offsetIncrements.rlshp), "The second rlshp", new Offset(321), new Offset(44), new Offset(23), new Offset(55), new Offset(5), new Offset(7)),
                new Rlshp(new Offset(offsetIncrements.rlshp * 2), "The third rlshp", new Offset(1), new Offset(1), new Offset(1), new Offset(1), new Offset(1), new Offset(1)),
            ]

            const rlshpsByteArray = new RlshpsEncoder(rlshps).write().content()

            assert.equal(offsetIncrements.rlshp * 3, rlshpsByteArray.length)
            ...
            assert.equal(offsetIncrements.rlshp, rlshp2.offset.offset)
            assert.equal("The second rlshp", rlshp2.label)
            assert.equal(321, rlshp2.firstNode)
            assert.equal(44, rlshp2.secondNode)


            assert.equal(offsetIncrements.rlshp * 2, rlshp3.offset.offset)
    ...
    describe(`Custom Prop ${offsetIncrements.prop} bytes binary encoding/decoding`, function () {
        it('Prop serde', async function () {
            const props = [
                new Prop(new Offset(0), "key1", "4d42aff95f8e7611386e8477f7d8d23a6ae08490f7922e03b50dbb08c9b308f9", new Offset(11)),
                new Prop(new Offset(offsetIncrements.prop), "key1", "efc8c0a704015f0d724f219e512b16b0574e4427162a0047acf2ee229fbdc529", new Offset(6543)),
                new Prop(new Offset(offsetIncrements.prop * 2), "key1", "d8eff270b2742d102a87536d81774845874894b479ef3729c904d375b60a999b", new Offset(77777))
            ]
            ...
            const propsEncoder = await new PropsEncoder(props, encode, put).write()
            const propsByteArray = propsEncoder.content()

            assert.equal(offsetIncrements.prop * 3, propsByteArray.length)

            const props2 = await new PropsDecoder(propsByteArray, decode, get).read()
            ...
            assert.equal(offsetIncrements.prop, p2.offset.offset)
            ...
            assert.equal(offsetIncrements.prop * 2, p3.offset.offset)
