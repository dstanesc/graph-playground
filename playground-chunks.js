...
const graphToChunky = async () => {
    ...
    console.log(`Node count ${g.nodes.size} expected size = ${g.nodes.size * 52}`)
    console.log(`Rlshp count ${g.rlshps.size} expected size = ${g.rlshps.size * 68}`)
    console.log(`Props count ${g.props.size} expected size = ${g.props.size * 116}`)
    const end = new Date()
